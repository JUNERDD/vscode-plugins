...
import {
  compareFileWithBranch,
  EMPTY_DOCUMENT_SCHEME,
  EmptyDocumentProvider,
} from "./branchFileDiff";
import { loadCommitSnapshot } from "./commitLoader";
import { registerDiffPreview } from "./diffPreview/provider";
import {
  getCommandUri,
  getGitApi,
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand(COMPARE_FILE_WITH_BRANCH_COMMAND, (argument?: unknown) =>
      openBranchFileDiff(argument),
    ),
  );
}
...
async function openBranchFileDiff(argument?: unknown): Promise<void> {
  try {
    await compareFileWithBranch(await getGitApi(), argument);
  } catch (error) {
    const detail = error instanceof Error ? error.message : String(error);
    void vscode.window.showErrorMessage(
      vscode.l10n.t("Unable to compare file with branch: {detail}", { detail }),
    );
  }
}
